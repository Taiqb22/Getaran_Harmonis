...
const dbkkm = db.ref('db_token/').on('value', kkmSuccess, handleError)
...
function kkmSuccess(items1) {
    ...
    const kkm1 = items1.val()[0]['token'];
    const kkm2 = items1.val()[1]['token'];
    const kkm3 = items1.val()[2]['token'];
...
function setKKM1() {
    ...
    let database = firebase.database().ref("db_token/" + 0);
    database.set(task);
}
...
function setKKM2() {
    ...
    let database = firebase.database().ref("db_token/" + 1);
    database.set(task);
}
...
function setKKM3() {
    ...
    let database = firebase.database().ref("db_token/" + 2);
    database.set(task);
}